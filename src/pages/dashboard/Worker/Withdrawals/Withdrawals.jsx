    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="animate-fade-in-down">
          <h1 className="text-3xl md:text-4xl font-bold font-heading text-secondary mb-2">
            Withdraw Earnings
          </h1>
          <p className="text-slate-500 text-lg">Convert your coins to real money</p>
        </div>
        ...
        {showSuccess && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-center gap-3 animate-fade-in shadow-sm">
            ...
            <div>
              <p className="text-emerald-800 font-bold">Withdrawal Request Submitted!</p>
        ...
        <div className="card glass-panel p-6 md:p-8 animate-fade-in-up">
          <h2 className="text-2xl font-bold text-secondary mb-6 flex items-center gap-2">
              <FaWallet className="text-primary" /> Your Balance
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Current Coins */}
            <div className="bg-gradient-to-br from-primary to-emerald-600 rounded-xl p-6 text-white shadow-lg relative overflow-hidden">
              ...
              <div className="flex items-center gap-4 mb-4 relative z-10">
                ...
                <div>
                  <p className="text-emerald-100 text-sm font-medium">Available Coins</p>
                  <p className="text-4xl font-bold">{userCoins}</p>
                </div>
              </div>
              <div className="h-px bg-white/20 my-4"></div>
              <p className="text-emerald-100 text-xs font-medium">Total earnings available for withdrawal</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl p-6 text-white shadow-lg relative overflow-hidden">
              ...
              <div className="flex items-center gap-4 mb-4 relative z-10">
                ...
                <div>
                  <p className="text-emerald-100 text-sm font-medium">Dollar Equivalent</p>
                  <p className="text-4xl font-bold">${withdrawalAmountInDollars.toFixed(2)}</p>
                </div>
              </div>
              <div className="h-px bg-white/20 my-4"></div>
              <p className="text-emerald-100 text-xs font-medium">20 coins = $1 (Current Rate)</p>
            </div>
          </div>

          {/* Minimum Withdrawal Info */}
          <div className="mt-6 bg-slate-100 border border-slate-200 rounded-xl p-4 flex items-start gap-3">
            <FaExclamationCircle className="w-5 h-5 text-slate-500 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-slate-800 font-bold text-sm">Minimum Withdrawal Requirement</p>
        ...
        {hasMinimumCoins ? (
          <form
            ...
            className="card glass-panel p-6 md:p-8 space-y-6 animate-fade-in-up delay-100"
          >
            <h2 className="text-2xl font-bold text-secondary mb-6 flex items-center gap-2">
                <FaMoneyCheckAlt className="text-primary" /> Withdrawal Request
            </h2>

            {/* Coins to Withdraw */}
            <div className="form-control w-full">
              <label className="label">
                <span className="label-text font-bold text-slate-700">Coins to Withdraw <span className="text-red-500">*</span></span>
              </label>
              <div className="relative">
                <FaCoins className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                <input
                  type="number"
                  value={coinsToWithdraw}
                  onChange={(e) => setCoinsToWithdraw(e.target.value)}
                  placeholder="Enter amount (min 200)"
                  className={`input input-bordered w-full pl-12 h-12 text-lg focus:ring-2 focus:ring-primary transition-all bg-white/50 ${
                    errors.coinsToWithdraw ? "input-error bg-red-50" : "border-slate-300 focus:border-primary"
                  }`}
                  min="200"
                  max={userCoins}
                />
              ...
              <label className="label">
                 <span className="label-text-alt text-slate-500">Max available: {userCoins} coins</span>
              </label>
            </div>
            ...
            <div className="form-control w-full">
              <label className="label">
                <span className="label-text font-bold text-slate-700">Withdrawal Amount ($)</span>
              </label>
              <div className="relative">
                <FaDollarSign className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                <input
                  type="text"
                  value={withdrawAmount.toFixed(2)}
                  readOnly
                  className="input input-bordered w-full pl-12 h-12 text-lg font-bold text-emerald-600 bg-slate-50 cursor-not-allowed"
                />
              </div>
              <label className="label">
                 <span className="label-text-alt text-slate-500">Auto-calculated (20 coins = $1)</span>
              </label>
            </div>
            ...
            <div className="form-control w-full">
              <label className="label">
                <span className="label-text font-bold text-slate-700">Select Payment System <span className="text-red-500">*</span></span>
              </label>
              <div className="relative">
                <FaCreditCard className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 z-10" />
                <select
                  value={paymentSystem}
                  onChange={(e) => setPaymentSystem(e.target.value)}
                  className={`select select-bordered w-full pl-12 h-12 text-base focus:ring-2 focus:ring-primary transition-all bg-white/50 ${
                    errors.paymentSystem ? "select-error bg-red-50" : "border-slate-300 focus:border-primary"
                  }`}
                >
            ...
            <div className="form-control w-full">
              <label className="label">
                <span className="label-text font-bold text-slate-700">Account Number <span className="text-red-500">*</span></span>
              </label>
              <div className="relative">
                <FaWallet className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                <input
                  type="text"
                  value={accountNumber}
                  ...
                  className={`input input-bordered w-full pl-12 h-12 text-lg focus:ring-2 focus:ring-primary transition-all bg-white/50 ${
                    errors.accountNumber ? "input-error bg-red-50" : "border-slate-300 focus:border-primary"
                  }`}
                />
              </div>
            ...
            <button
              ...
              className="btn btn-block h-14 btn-primary-gradient border-none text-white font-bold text-lg shadow-lg transform hover:-translate-y-0.5 transition-all disabled:opacity-70"
            >
              {isSubmitting ? (
          ...
          <div
            className="card glass-panel p-8 border-red-100 text-center animate-fade-in-up"
          >
            ...
            <h3 className="text-2xl font-bold text-secondary mb-2">Insufficient Coins</h3>
            <p className="text-slate-600 mb-8 max-w-md mx-auto">
              You need at least <span className="font-bold text-primary">200 coins ($10)</span> to make a withdrawal. You currently have{" "}
              <span className="font-bold text-primary">{userCoins} coins</span>.
            </p>
            
            <Link
              to="/dashboard/task-list"
              className="inline-flex items-center gap-2 px-8 py-4 btn-primary-gradient text-white font-bold rounded-xl hover:shadow-lg hover:scale-105 transition-all duration-300"
            >
        ...
        {/* Business Logic Info */}
        <div
          className="rounded-2xl shadow-xl p-6 md:p-8 text-white animate-fade-in-up delay-200 relative overflow-hidden bg-gradient-to-r from-secondary to-slate-900"
        >
             <div className="absolute right-0 bottom-0 w-64 h-64 bg-primary opacity-20 rounded-full translate-x-1/3 translate-y-1/3 blur-2xl"></div>
          <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
              <FaCheckCircle className="text-emerald-400"/> How Withdrawals Work
          </h3>
          <ul className="space-y-3 text-slate-300">
            <li className="flex items-start gap-3 bg-white/5 p-3 rounded-lg backdrop-blur-md">
              <FaCheckCircle className="w-4 h-4 mt-1 flex-shrink-0 text-emerald-400" />
